...
// AWS 매분자료 주요 관측소 (STN코드, 도시명, 위경도)
const AWS_STATIONS = [
  {stn:108,name:'서울',lat:37.57,lng:126.97},{stn:159,name:'부산',lat:35.10,lng:129.03},
  {stn:143,name:'대구',lat:35.88,lng:128.62},{stn:112,name:'인천',lat:37.48,lng:126.62},
  {stn:156,name:'광주',lat:35.17,lng:126.89},{stn:133,name:'대전',lat:36.37,lng:127.37},
  {stn:152,name:'울산',lat:35.56,lng:129.32},{stn:119,name:'수원',lat:37.27,lng:126.99},
  {stn:155,name:'창원',lat:35.17,lng:128.57},{stn:146,name:'전주',lat:35.82,lng:127.15},
  {stn:131,name:'청주',lat:36.64,lng:127.44},{stn:138,name:'포항',lat:36.03,lng:129.38},
  {stn:184,name:'제주',lat:33.51,lng:126.53},{stn:105,name:'강릉',lat:37.75,lng:128.89},
  {stn:175,name:'진주',lat:35.16,lng:128.04},{stn:165,name:'목포',lat:34.82,lng:126.38},
];
...
export default {
  ...
  async fetch(req, env, ctx) {
    ...
    // /weather — AWS 매분자료 (기온·습도·풍속·풍향) + 태풍정보
    if (url.pathname === '/weather') {
      const kst = new Date(Date.now() + 9*3600000);
      const pad = n => String(n).padStart(2,'0');
      // 10분 전 관측시각 (최신 데이터 확보)
      ...
      // 16개 관측소 병렬 조회
      const awsResults = await Promise.all(AWS_STATIONS.map(async s => {
        try {
          const r = await fetch(
            `https://apihub.kma.go.kr/api/typ01/cgi-bin/url/nph-aws2_min?tm2=${tm}&stn=${s.stn}&help=0&authKey=${env.KMA_AUTH_KEY}`,
            { signal: AbortSignal.timeout(8000) }
          );
          const txt = await r.text();
          const line = txt.split('\n').find(l => l.trim() && !l.startsWith('#'));
          if (!line) return { ...s, ta: null, hm: null, ws: null, wd: null, rn: null };
          const f = line.trim().split(/\s+/);
          // 컬럼: tm stn WD1 WS1 WDS WSS WD10 WS10 TA RE RN-15 RN-60 RN-12 RN-DAY HM PA PS TD
          const v = x => { const n = parseFloat(x); return (isNaN(n)||n<-50) ? null : n; };
          return { ...s, ta: v(f[8]), hm: v(f[14]), ws: v(f[3]), wd: v(f[2]), rn: v(f[10]) };
        } catch { return { ...s, ta: null, hm: null, ws: null, wd: null, rn: null }; }
      }));
      ...
      try {
        ...
        typhoons = ttxt.split('\n')
          ...
          .map(l => {
            ...
            return { yr:f[0], seq:f[1], active:f[2]==='1', eff:f[3], name_kr:f[6], name_en:f[7] };
          })
          .filter(t => t.active); // 현재 활동 중인 태풍만
      } catch {}
